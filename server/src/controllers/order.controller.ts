...
// ✅ FIXED: Create order with complete product data including SKU and image
export const createOrder = async (req: Request, res: Response) => {
  try {
    ...
    const result = await runTransaction(async () => {
      ...
      // ✅ CRITICAL FIX: Process each item to ensure ALL fields are present including SKU and image
      const processedItems = req.body.items.map((item: any) => ({
        productId: item.productId,
        title: item.title || 'Product',
        price: parseFloat(item.price) || 0,
        // ✅ CRITICAL: Save image URL
        image: item.image || '',
      ...
      console.log('✅ Item details with SKU and image:', processedItems.map(item => ({
        title: item.title,
        sku: item.sku,
        hasImage: !!item.image,
        size: item.size,
...
// ✅ FIXED: Get orders with complete data including SKU and image
export const getOrders = async (req: Request, res: Response) => {
  try {
    ...
    // ✅ CRITICAL FIX: Ensure all order items have the necessary fields including SKU and image
    const processedOrders = orders.map(order => ({
      ...
      items: order.items.map(item => ({
        ...
        image: item.image || '',
        imageUrl: item.image || '', // Add imageUrl for compatibility
        thumbnail: item.image || '' // Add thumbnail for compatibility
      }))
    }));
...
export const getOrder = async (req: Request, res: Response) => {
  try {
    ...
    // ✅ FIXED: Process items to ensure all fields
