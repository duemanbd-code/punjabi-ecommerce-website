...
const convertToFacebookCatalogCSV = (products: Product[]): string => {
  ...
  const headers = [
    ...
    'age_group', 'material', 'pattern', 'shipping', 'shipping_weight', 'video[0].url', 'gtin',
  ...
  const rows = products.map(product => {
    ...
    return [
      escapeField(product._id), escapeField(product.title),
      escapeField(product.description.replace(/\n/g, ' ').substring(0, 9999)),
      availability, 'new', price, escapeField(productUrl), escapeField(imageUrl),
      escapeField(product.brand || 'DuemanBD'), googleCategory, fbCategory,
      product.stockQuantity > 0 ? String(product.stockQuantity) : '', salePrice, '',
      '', product.gender || 'unisex', escapeField(product.color || ''), escapeField(product.size || ''),
      product.age_group || 'adult', escapeField(product.material || ''), escapeField(product.pattern || ''),
      'BD:All:Standard:50 BDT', product.weight ? escapeField(product.weight) : '',
      product.videoUrl ? escapeField(product.videoUrl) : '', product.gtin ? escapeField(product.gtin) : '',
      escapeField(productTag1), escapeField(productTag2), escapeField(style)
    ];
  });
...
const exportAllProductsToFacebookCatalog = async (token: string, totalProducts: number, setExporting: (loading: boolean) => void) => {
  try {
    ...
    const discountedProducts = allProducts.filter(p => {
      ...
      return hasSalePrice && p.salePrice < p.normalPrice;
    });
...
export default function AdminProductsPage() {
  ...
  // ✅ FIXED: Removed checkAuthAndRedirect - auth is handled by ProtectedRoute in layout
  useEffect(() => {
    fetchProducts(1);
  }, []);
