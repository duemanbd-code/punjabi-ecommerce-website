// client/src/app/all-collections/page.tsx - UPDATED with Pagination
...
import { fetchAllProducts, PaginatedResponse } from "@/lib/utils/product.api";
...
const AllCollectionsPage = () => {
  ...
  const fetchProducts = async (page: number = 1) => {
    try {
      ...
      const result = await fetchAllProducts(page, pagination.limit, {
        category: selectedCategory !== 'all' ? selectedCategory : undefined,
        search: searchQuery || undefined,
        minPrice: priceRange[0],
        maxPrice: priceRange[1],
        sortBy: selectedSort === 'price-low' || selectedSort === 'price-high' ? 'normalPrice' : 
                selectedSort === 'newest' ? 'createdAt' : 
                selectedSort === 'rating' ? 'rating' : 'createdAt',
        sortOrder: selectedSort === 'price-low' ? 'asc' : 
                   selectedSort === 'price-high' ? 'desc' : 'desc'
      });
      ...
      // Apply rating filter
      if (ratingFilter > 0) {
        filtered = filtered.filter(p => p.rating && p.rating >= ratingFilter);
      }
      ...
      if (showBestSellers) {
        filtered = filtered.filter(p => p.isBestSelling);
      }
      ...
      if (showDiscount) {
        filtered = filtered.filter(p => p.offerPrice && p.offerPrice < p.normalPrice);
      }
          ...
          <div className="lg:w-1/4 hidden lg:block">
            <div className="sticky top-8">
              <ProductFilters
                ...
                discountCount={products.filter(p => p.offerPrice && p.offerPrice < p.normalPrice).length}
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                ...
                {priceRange[1] < 100000 && (
                  <div className="inline-flex items-center bg-amber-50 text-amber-700 px-3 py-1.5 rounded-full text-sm">
                    ...
                    <button
                      onClick={() => handlePriceRangeChange([priceRange[0], 100000])}
                      className="ml-2 hover:text-amber-800"
                    >
                      <X className="w-3 h-3" />
                    </button>
...
//   // Filters - NOW INCLUDES ALL FILTERS
//   const [selectedCategory, setSelectedCategory] = useState<string>("all");
//   const [selectedSort, setSelectedSort] = useState<string>("featured");
//   const [priceRange, setPriceRange] = useState<[number, number]>([0, 100000]);
//   const [ratingFilter, setRatingFilter] = useState<number>(0);
...
//     // Apply best sellers filter - NEW
//     if (showBestSellers) {
//       result = result.filter((product) => product.isBestSelling);
//     }

//     // Apply new arrivals filter - NEW
//     if (showNewArrivals) {
//       result = result.filter((product) => product.isNew);
//     }

//     // Apply discount filter - NEW
//     if (showDiscount) {
//       result = result.filter(
//         (product) =>
...
//   // Reset filters - NOW INCLUDES ALL FILTERS
...
//   // Get active filter count - NOW INCLUDES ALL FILTERS
//   const activeFilterCount = useMemo(() => {
//     let count = 0;
//     if (selectedCategory !== "all") count++;
//     if (selectedSort !== "featured") count++;
...
//   // Collection stats - SIMILAR TO CATEGORY PAGE
...
//       <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
//         <div className="flex flex-col lg:flex-row gap-8">
//           {/* Sidebar Filters - Desktop - NOW USING ProductFilters COMPONENT */}
//           <div className="lg:w-1/4 hidden lg:block">
//             <div className="sticky top-8">
//               <ProductFilters
//                 categories={categories}
...
//             {/* Active Filters - NOW INCLUDES ALL FILTERS */}
//             {activeFilterCount > 0 && (
//               <div className="flex flex-wrap gap-2 mb-6">
//                 {selectedCategory !== "all" && (
//                   <div className="inline-flex items-center bg-amber-50 text-amber-700 px-3 py-1.5 rounded-full text-sm">
...
//                 {/* Stats Footer - SAME STYLE AS CATEGORY PAGE */}
...
//       {/* Mobile Filters Modal - NOW USING ProductFilters COMPONENT */}
//       {showMobileFilters && (
//         <div className="fixed inset-0 z-50 overflow-hidden lg:hidden">
//           <div
//             className="absolute inset-0 bg-black/50"
