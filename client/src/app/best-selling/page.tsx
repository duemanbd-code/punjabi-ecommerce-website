// client/src/app/best-selling/page.tsx - COMPLETE UPDATED FILE WITH PAGINATION
...
const BestSellingPage = () => {
  // State variables - initialize with empty arrays
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Use fetchAllProducts with pagination
        ...
        // Log first product to see fields
        if (allProducts.length > 0) {
          console.log("🔍 Sample product fields:", Object.keys(allProducts[0]));
        }

        // Filter only best selling products - try multiple field names
        const bestSellingProducts = allProducts.filter((product: any) => 
          product.isBestSelling === true || 
          product.isBestSeller === true ||
          product.bestSelling === true ||
          (product.rating && product.rating >= 4.5) // Fallback: high rating
        );
        
        console.log(`⭐ Found ${bestSellingProducts.length} best selling products`);
        
        // If no best selling products, show top rated as fallback
        let productsToShow = bestSellingProducts;
        if (bestSellingProducts.length === 0 && allProducts.length > 0) {
          console.log("⚠️ No best selling products found, showing top rated instead");
        ...
        setError(error.message || "Failed to load best selling products");
      } finally {
        setLoading(false);
      }
    };
  ...
  // Check if product has selected size
  ...
  // Apply filters - SAFELY handle arrays
  useEffect(() => {
  ...
  // Get paginated products - SAFELY handle filteredProducts
  ...
  // Get total pages - SAFELY handle filteredProducts
  ...
  const stats = useMemo(() => {
    ...
    const discountProducts = productsArray.filter(
      (p) => p.offerPrice && p.offerPrice < (p.normalPrice || 0)
    );

    return {
      total: productsArray.length,
      avgRating: Math.round(avgRating * 10) / 10,
      highRated: productsArray.filter((p) => p.rating && p.rating >= 4.5).length,
      categories: categories.length - 1,
      newArrivals: productsArray.filter((p) => p.isNew).length,
  ...
  // SAFETY: Ensure filteredProducts is an array
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-1/4 hidden lg:block">
            <ProductFilters
              ...
              bestSellersCount={safeProducts.filter((p) => p.isBestSelling).length}
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  Showing {safeFilteredProducts.length} of {safeProducts.length} best selling products
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                ...
                {priceRange[1] < maxPrice && (
                  <div className="inline-flex items-center bg-amber-50 text-amber-700 px-3 py-1.5 rounded-full text-sm">
                    ...
                    <button
                      onClick={() => handlePriceRangeChange([priceRange[0], maxPrice])}
                      className="ml-2 hover:text-amber-800"
                    >
                      <X className="w-3 h-3" />
                    </button>
