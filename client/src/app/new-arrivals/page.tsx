// client/src/app/new-arrivals/page.tsx - COMPLETE UPDATED FILE WITH PAGINATION
...
import { fetchAllProducts } from "@/lib/utils/product.api";
...
const NewArrivalsPage = () => {
  ...
  const [showNewArrivals, setShowNewArrivals] = useState<boolean>(true);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter only new arrivals - try multiple field names
        const newArrivalsProducts = allProducts.filter((product: any) => 
          product.isNew === true || 
          product.isNewProduct === true ||
          product.newArrival === true
        );
        
        console.log(`🌟 Found ${newArrivalsProducts.length} new arrivals`);
        
        // If no new arrivals, show most recent as fallback
        let productsToShow = newArrivalsProducts;
        if (newArrivalsProducts.length === 0 && allProducts.length > 0) {
          console.log("⚠️ No new arrivals found, showing most recent instead");
        ...
        const processedProducts = productsToShow.map((product: any) => ({
          ...product,
  ...
  const hasSelectedSize = useCallback(
    (product: Product, selectedSizes: string[]): boolean => {
      ...
      return product.sizes.some((size: any) => {
        ...
        return sizeValue && selectedSizes.includes(sizeValue);
      });
    },
    []
  );
  ...
  useEffect(() => {
    ...
    result = result.filter(
      (product) =>
        (product.normalPrice || 0) >= priceRange[0] &&
        (product.normalPrice || 0) <= priceRange[1]
    );
    ...
    result = result.filter((product) => {
      ...
      if (showDiscount && (!product.offerPrice || product.offerPrice >= (product.normalPrice || 0)))
        return false;
      return true;
    });
    ...
    switch (selectedSort) {
      case "price-low":
        result.sort((a, b) => (a.normalPrice || 0) - (b.normalPrice || 0));
        break;
      case "price-high":
        result.sort((a, b) => (b.normalPrice || 0) - (a.normalPrice || 0));
        break;
      case "rating":
  ...
  const maxPrice = useMemo(() => {
    const productsArray = Array.isArray(products) ? products : [];
    return Math.max(...productsArray.map((p) => p.normalPrice || 0), 1000);
  }, [products]);
  ...
  const totalPages = useMemo(() => {
    const productsArray = Array.isArray(filteredProducts) ? filteredProducts : [];
    return Math.ceil(productsArray.length / itemsPerPage) || 1;
  ...
  const stats = useMemo(() => {
    ...
    const avgRating =
      productsArray.length > 0
        ? productsArray.reduce((acc, p) => acc + (p.rating || 0), 0) /
    ...
    return {
      ...
      bestSellers: productsArray.filter((p) => p.isBestSelling).length,
    };
  }, [products, categories]);
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="text-center mb-12">
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6">
              Latest <span className="text-amber-600">Collections</span>
            </h1>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="text-center mb-12">
            <div className="bg-red-50 border border-red-200 rounded-xl p-8 max-w-md mx-auto">
              ...
              <button onClick={() => window.location.reload()} className="px-6 py-3 bg-gradient-to-r from-slate-950 to-amber-500 text-white rounded-lg">
                Try Again
              </button>
            </div>
          </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
          <div className="max-w-4xl mx-auto text-center">
            <div className="inline-flex items-center px-4 py-2 bg-gradient-to-r from-amber-500 to-orange-500 rounded-full mb-6">
              <Sparkles className="w-4 h-4 text-white mr-2" fill="white" />
              <span className="text-white font-semibold">New Arrivals</span>
            </div>
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6">
              Latest <span className="text-amber-400">Collections</span>
            </h1>
            <div className="relative max-w-2xl mx-auto">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-4 rounded-full bg-white text-slate-800 border border-white/20 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-500"
                />
                {searchQuery && (
                  <button
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="lg:w-1/4 hidden lg:block">
            <ProductFilters
              ...
              bestSellersCount={stats.bestSellers}
              newArrivalsCount={safeProducts.filter((p) => p.isNew).length}
              discountCount={safeProducts.filter((p) => p.offerPrice && p.offerPrice < (p.normalPrice || 0)).length}
          ...
          <div className="lg:w-3/4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  Showing {safeFilteredProducts.length} of {safeProducts.length} new products
                </p>
              </div>
              ...
              <div className="flex items-center gap-4">
                <div className="relative">
                  <input
                    ...
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 w-64"
                  />
                ...
                <div className="flex bg-gray-100 rounded-lg p-1">
                  <button
                    ...
                    className={`p-2 rounded-md transition-colors ${
                      viewMode === "grid" ? "bg-white text-amber-600 shadow-sm" : "text-gray-500 hover:text-gray-700"
                    }`}
                  >
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {selectedCategory !== "all" && (
                  <div className="inline-flex items-center bg-amber-50 text-amber-700 px-3 py-1.5 rounded-full text-sm">
                    ...
                    <button onClick={() => setSelectedCategory("all")} className="ml-2 hover:text-amber-800">
                      <X className="w-3 h-3" />
                    </button>
                  </div>
                )}
                <button onClick={resetFilters} className="inline-flex items-center text-sm text-gray-600 hover:text-amber-600">
                  Clear all
                </button>
              </div>
            )}
            ...
            {safeFilteredProducts.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-2xl border border-gray-200">
                ...
                <h3 className="text-xl font-semibold text-gray-900 mb-3">No New Arrivals Found</h3>
                <button onClick={resetFilters} className="px-6 py-3 bg-gradient-to-r from-slate-950 to-amber-500 text-white rounded-lg">
                  Clear All Filters
                </button>
              </div>
            ) : (
                  ...
                  <div className="mt-12 pt-8 border-t border-gray-200">
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                      <div className="text-gray-600 text-sm">
                        Page {currentPage} of {totalPages} • {safeFilteredProducts.length} products
                      </div>
            ...
            <div className="relative w-screen max-w-md">
              <div className="flex h-full flex-col bg-white shadow-xl">
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                  ...
                  <button onClick={() => setShowMobileFilters(false)} className="p-2 text-gray-400 hover:text-gray-500">
                    <X className="w-6 h-6" />
                  </button>
                </div>
...
//   const [selectedSizes, setSelectedSizes] = useState<string[]>([]);
//   const [showBestSellers, setShowBestSellers] = useState<boolean>(false);
//   const [showNewArrivals, setShowNewArrivals] = useState<boolean>(true); // Always true for this page
//   const [showDiscount, setShowDiscount] = useState<boolean>(false);
//   const [searchQuery, setSearchQuery] = useState<string>("");
...
//         // Filter only new arrivals products
//         const newArrivalsProducts = productData.filter(
//           (product) => product.isNew
//         );
        
//         // Process image URLs to ensure they're full URLs
//         const processedProducts = newArrivalsProducts.map((product: any) => ({
//           ...product,
//           imageUrl: getFullImageUrl(product.imageUrl || product.image || product.thumbnail)
//         }));
...
//         // Extract unique sizes - CORRECTED VERSION
//         const uniqueSizes = new Set<string>();
//         processedProducts.forEach((product) => {
//           if (product.sizes && Array.isArray(product.sizes)) {
//             product.sizes.forEach((size: any) => {
...
//         } else {
//           setError(error.message || "Failed to load new arrivals products. Please try again later.");
//         }
//       } finally {
//         setLoading(false);
//       }
...
//     // Apply product status filters (always show new arrivals for this page)
//     result = result.filter((product) => {
//       if (showBestSellers && !product.isBestSelling) return false;
//       if (showNewArrivals && !product.isNew) return false;
//       if (
...
//       case "featured":
//         result.sort((a, b) => {
//           // Sort by newness + rating
//           const scoreA = (a.isNew ? 100 : 0) + (a.rating || 0) * 10;
//           const scoreB = (b.isNew ? 100 : 0) + (b.rating || 0) * 10;
...
//                 </h2>
//                 <p className="text-gray-600 mt-1">
//                   Showing {filteredProducts.length} of {products.length} new
//                   products
//                 </p>
//               </div>
...
//                   {searchQuery
//                     ? `No new arrivals found for "${searchQuery}". Try different keywords or clear filters.`
//                     : "No new arrivals match your current filters. Try adjusting your criteria."}
//                 </p>
//                 <button
//                   onClick={resetFilters}
//                   className="px-6 py-3 bg-gradient-to-r from-slate-950 to-amber-500 text-white rounded-lg hover:from-slate-800 hover:to-amber-600 transition-colors font-medium"
