...
import {
  ...
  List,
  Key,
  Maximize2, // Added for size guide
} from "lucide-react";
...
export default function ProductDetailPage() {
  ...
  // Function to push view_item event to dataLayer
  const pushViewItemToDataLayer = (productData: any) => {
    ...
    try {
      ...
      const currentPrice = productData.salePrice && productData.salePrice < productData.normalPrice 
        ? productData.salePrice 
        : (productData.offerPrice && productData.offerPrice < productData.normalPrice 
      ...
      window.dataLayer.push({
        ...
        ecommerce: {
          ...
          value: currentPrice * (quantity || 1),
          items: [
            {
              item_id: productData.sku || productData._id || "SKU_" + productData._id,
              item_name: productData.title || "Product",
              affiliation: "Puti Client Store",
              coupon: "",
              discount: productData.normalPrice && productData.salePrice 
                ? (productData.normalPrice - productData.salePrice) * (quantity || 1)
                : 0,
              index: 0,
              item_brand: productData.brand || "Puti",
              ...
              item_variant: selectedSize || "default",
              location_id: "",
              price: currentPrice,
              quantity: quantity || 1
            }
      ...
      console.log('✅ view_item event pushed to dataLayer:', {
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        // UPDATED: Added all product fields including features and specifications
        const processedProduct = {
          ...
          sku: productData.sku || "", // Product code from admin
  ...
  // ==================== DATA LAYER TRIGGER ====================
  // Push view_item event when product data is loaded
  useEffect(() => {
    if (product && !loading) {
      // Small delay to ensure all state is ready
      const timer = setTimeout(() => {
        pushViewItemToDataLayer(product);
      }, 500);
      
      return () => clearTimeout(timer);
    }
  }, [product, loading, selectedSize, quantity]);
  ...
  // Simple Size Guide Modal Component - Just shows the image
        ...
        <div className="relative max-w-4xl w-full max-h-[90vh] bg-white rounded-2xl overflow-hidden">
          ...
          <div className="flex items-center justify-between p-4 sm:p-6 border-b border-slate-200 bg-white">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">Find your perfect fit</p>
              </div>
            </div>
            <button
          ...
          <div className="overflow-y-auto max-h-[calc(90vh-80px)] p-4">
            <div className="rounded-xl overflow-hidden border border-slate-200 shadow-lg bg-white">
              <img
                src="/size-guide.png" // Your image from public folder
                alt="Size Guide"
                className="w-full h-auto object-contain max-h-[70vh]"
                onError={(e) => {
                  console.error("Failed to load size guide image");
                  e.currentTarget.src = "https://images.unsplash.com/photo-1583743814966-8936f5b7be1a?w=1200&h=1500&fit=crop&auto=format";
                }}
              />
            </div>
          </div>
          ...
          <div className="p-4 sm:p-6 border-t border-slate-200 bg-white">
            <div className="flex justify-center">
              <button
                onClick={() => setShowSizeGuide(false)}
                className="px-6 py-3 bg-gradient-to-r from-amber-500 to-amber-600 text-white font-semibold rounded-lg hover:from-amber-600 hover:to-amber-700 transition-all shadow-lg hover:shadow-xl"
              >
                Close Size Guide
              </button>
            </div>
  ...
  // ==================== UPDATED DESCRIPTION TAB CONTENT ====================
      ...
      case "description":
        return (
          <div className="space-y-8">
                ...
                <div className="space-y-4">
                  ...
                  {product?.sku && (
                    <div className="flex items-start justify-between py-2 border-b border-slate-100">
                      ...
                      <span className="text-slate-900 font-mono bg-slate-100 px-3 py-1 rounded-lg text-sm">
                        {product.sku}
                      </span>
                    </div>
                  )}
                  ...
                  {product?.sizes && product.sizes.length > 0 && (
                    <div className="flex items-start justify-between py-2 border-b border-slate-100">
                      <span className="text-slate-600 font-medium">Available Sizes</span>
                      <div className="flex flex-wrap gap-2 justify-end">
                        {product.sizes.map((size: any, idx: number) => (
                          <span
                            key={idx}
                            className="px-2 py-1 bg-amber-100 text-amber-700 rounded-md text-sm font-medium"
                          >
                            {size.size}
                          </span>
                        ))}
            ...
            {/* Additional Information Grid - If needed */}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                ...
                {product?.returnPolicy && (
                  <div className="bg-gradient-to-b from-slate-50 to-white rounded-xl border border-slate-200 p-4">
                    ...
                    <p className="text-sm text-slate-600">{product.returnPolicy}</p>
                  </div>
                )}
                ...
                <div className="bg-gradient-to-b from-slate-50 to-white rounded-xl border border-slate-200 overflow-hidden">
                  <div className="divide-y divide-slate-200">
                    {Object.entries(product.specifications).map(([key, value]: [string, any], index) => (
                      <div key={index} className="grid grid-cols-3 gap-4 p-4 hover:bg-slate-50 transition-colors">
            ...
            {product?.tags && product.tags.length > 0 && (
              <div className="mt-6 pt-6 border-t border-slate-200">
                <h4 className="text-sm font-medium text-slate-600 mb-3">Product Tags</h4>
                <div className="flex flex-wrap gap-2">
                  {product.tags.map((tag: string, index: number) => (
                    <Link
                      key={index}
                      href={`/products?tag=${encodeURIComponent(tag)}`}
                      className="px-3 py-1.5 bg-slate-100 text-slate-700 rounded-lg text-sm hover:bg-amber-100 hover:text-amber-700 transition-colors"
                    >
                      #{tag}
                    </Link>
                  ))}
          ...
          <div className="space-y-6 sm:space-y-8">
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0 flex-1">
                ...
                {/* UPDATED: Product Code Display - Changed "Code" to "SKU" */}
              ...
              <div className="space-y-2 sm:space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-amber-600 hover:text-amber-700 font-medium text-sm flex items-center gap-1 transition-colors"
                  >
                    <Maximize2 size={14} />
                    Size Guide
                  </button>
