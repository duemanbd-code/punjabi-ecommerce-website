...
import { 
  ...
  ChevronDown, Filter, Sparkles, Award, Zap, Percent,
  Package, Shirt
} from "lucide-react";
...
const categoryHierarchy: Record<string, { name: string; parent?: string; icon?: string; description: string }> = {
  "all": { name: "All Categories", description: "Browse all products across every category", icon: "📦" },
  "panjabi": { name: "Panjabi", description: "Traditional Panjabi collection for every occasion", icon: "👔" },
  "regular-panjabi": { name: "Regular Panjabi", parent: "panjabi", description: "Comfortable regular fit Panjabi for daily wear", icon: "👕" },
  "premium-panjabi": { name: "Premium Panjabi", parent: "panjabi", description: "Premium quality Panjabi with fine fabric", icon: "✨" },
  "luxury-panjabi": { name: "Luxury Panjabi", parent: "panjabi", description: "Luxury Panjabi for special occasions", icon: "👑" },
  "shirts": { name: "Shirts", description: "Premium quality shirts collection", icon: "👔" },
};

const fixedCategories = [
  { slug: "all", name: "All Categories", description: "Browse all products across every category" },
  { slug: "panjabi", name: "Panjabi", description: "Traditional Panjabi collection for every occasion" },
  { slug: "regular-panjabi", name: "Regular Panjabi", description: "Comfortable regular fit Panjabi for daily wear", parentCategory: "panjabi" },
  { slug: "premium-panjabi", name: "Premium Panjabi", description: "Premium quality Panjabi with fine fabric", parentCategory: "panjabi" },
  { slug: "luxury-panjabi", name: "Luxury Panjabi", description: "Luxury Panjabi for special occasions", parentCategory: "panjabi" },
  { slug: "shirts", name: "Shirts", description: "Premium quality shirts collection" },
];
...
export default function CategoryPage() {
  ...
  const breadcrumbItems = useMemo(() => {
    ...
    if (slug !== "all") {
      const category = categoryHierarchy[slug];
      if (category?.parent) {
        items.push({ name: categoryHierarchy[category.parent].name, slug: category.parent });
      }
    }
    
    if (categoryInfo) {
      items.push({ name: categoryInfo.name, slug: categoryInfo.slug });
    }
  ...
  useEffect(() => {
    const fetchCategoryProducts = async () => {
      try {
        ...
        const categoryData = categoryHierarchy[slug];
        ...
        } else if (slug === "panjabi") {
          // Get all panjabi subcategories
          const panjabiSubs = ["regular-panjabi", "premium-panjabi", "luxury-panjabi"];
          const promises = panjabiSubs.map(sub => 
            axios.get(`${API_URL}/products/category/${sub}`, {
              timeout: 15000,
              headers: { 'Accept': 'application/json', 'Cache-Control': 'no-cache' }
            }).catch(() => ({ data: { success: true, data: [] } }))
          );
          const results = await Promise.all(promises);
          const allProducts = results.flatMap(r => r.data.data || []);
          response = { data: { success: true, data: allProducts } };
        } else {
          response = await axios.get(`${API_URL}/products/category/${slug}`, {
            timeout: 15000,
            headers: { 'Accept': 'application/json', 'Cache-Control': 'no-cache' }
          });
        }
  ...
  useEffect(() => {
    ...
    if (ratingFilter > 0) {
      filtered = filtered.filter(product => product.rating && product.rating >= ratingFilter);
    }
    ...
    products, selectedCategory, selectedSort, priceRange, ratingFilter, 
    selectedSizes, showBestSellers, showNewArrivals, showDiscount, searchQuery
  ]);
  ...
  const activeFilterCount = useMemo(() => {
    ...
    return count;
  }, [selectedCategory, selectedSort, priceRange, maxPrice, ratingFilter, selectedSizes, showBestSellers, showNewArrivals, showDiscount, searchQuery]);

  const collectionStats = useMemo(() => {
    const discountProducts = products.filter(p => p.originalPrice && p.originalPrice > p.normalPrice);
    ...
    return {
      ...
      avgRating: products.length > 0 ? products.reduce((acc, p) => acc + (p.rating || 0), 0) / products.length : 0,
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white py-12">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-gray-500 text-sm mt-2">Loading from: {getApiBaseUrl()}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white flex items-center justify-center p-6">
        <div className="text-center max-w-md">
          ...
          <div className="flex flex-col gap-3">
            <Link href="/" className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-slate-950 to-amber-500 text-white rounded-lg hover:from-slate-800 hover:to-amber-600 transition-all font-medium justify-center">
              <ArrowRight className="rotate-180" size={18} />
              Back to Home
            </Link>
            <button onClick={() => window.location.reload()} className="inline-flex items-center gap-2 px-6 py-3 border border-slate-300 text-slate-700 rounded-lg hover:bg-slate-50 transition-all font-medium justify-center">
              Try Again
            </button>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
          <div className="max-w-3xl mx-auto text-center">
            <div className="inline-flex items-center px-4 py-2 bg-white/10 backdrop-blur-sm rounded-full mb-6">
              <span className="text-2xl mr-2">{categoryInfo?.icon || (slug === "panjabi" ? "👔" : slug === "shirts" ? "👕" : "📦")}</span>
            ...
            <div className="relative max-w-2xl mx-auto">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-4 rounded-full bg-white text-slate-800 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                />
                {searchQuery && (
                  <button onClick={() => setSearchQuery("")} className="absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-800 hover:text-slate-950">
                    <X className="w-5 h-5" />
                  </button>
                )}
              </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex bg-gray-100 rounded-lg p-1">
                  <button onClick={() => setViewMode("grid")} className={`p-2 rounded-md transition-colors ${viewMode === "grid" ? "bg-white text-amber-600 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}>
                    <Grid className="w-5 h-5" />
                  </button>
                  <button onClick={() => setViewMode("list")} className={`p-2 rounded-md transition-colors ${viewMode === "list" ? "bg-white text-amber-600 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}>
                ...
                {/* Sort Dropdown */}
                <div className="relative">
                  <select value={selectedSort} onChange={(e) => setSelectedSort(e.target.value)} className="appearance-none bg-white border border-gray-300 rounded-lg pl-4 pr-10 py-2.5 text-gray-700 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent cursor-pointer">
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {selectedCategory !== "all" && (
                  <div className="inline-flex items-center bg-amber-50 text-amber-700 px-3 py-1.5 rounded-full text-sm">
                    ...
                    <button onClick={() => setSelectedCategory("all")} className="ml-2 hover:text-amber-800"><X className="w-3 h-3" /></button>
                  </div>
                )}
                {priceRange[0] > 0 && (
                  <div className="inline-flex items-center bg-amber-50 text-amber-700 px-3 py-1.5 rounded-full text-sm">
                    Min: ৳{priceRange[0].toLocaleString()}
                    <button onClick={() => setPriceRange([0, priceRange[1]])} className="ml-2 hover:text-amber-800"><X className="w-3 h-3" /></button>
                  </div>
                )}
                {priceRange[1] < maxPrice && (
                  <div className="inline-flex items-center bg-amber-50 text-amber-700 px-3 py-1.5 rounded-full text-sm">
                    Max: ৳{priceRange[1].toLocaleString()}
                    <button onClick={() => setPriceRange([priceRange[0], maxPrice])} className="ml-2 hover:text-amber-800"><X className="w-3 h-3" /></button>
                  </div>
                )}
                {ratingFilter > 0 && (
                  <div className="inline-flex items-center bg-amber-50 text-amber-700 px-3 py-1.5 rounded-full text-sm">
                    {ratingFilter}+ Stars
                    <button onClick={() => setRatingFilter(0)} className="ml-2 hover:text-amber-800"><X className="w-3 h-3" /></button>
                  </div>
                )}
                {selectedSizes.map(size => (
                  <div key={size} className="inline-flex items-center bg-amber-50 text-amber-700 px-3 py-1.5 rounded-full text-sm">
                    Size: {size}
                    <button onClick={() => toggleSizeSelection(size)} className="ml-2 hover:text-amber-800"><X className="w-3 h-3" /></button>
                  </div>
                ))}
                {showBestSellers && (
                  <div className="inline-flex items-center bg-amber-50 text-amber-700 px-3 py-1.5 rounded-full text-sm">
                    <Award className="w-3 h-3 mr-1" /> Best Sellers
                    <button onClick={() => setShowBestSellers(false)} className="ml-2 hover:text-amber-800"><X className="w-3 h-3" /></button>
                  </div>
                )}
                {showNewArrivals && (
                  <div className="inline-flex items-center bg-amber-50 text-amber-700 px-3 py-1.5 rounded-full text-sm">
                    <Zap className="w-3 h-3 mr-1" /> New Arrivals
                    <button onClick={() => setShowNewArrivals(false)} className="ml-2 hover:text-amber-800"><X className="w-3 h-3" /></button>
                  </div>
                )}
                {showDiscount && (
                  <div className="inline-flex items-center bg-amber-50 text-amber-700 px-3 py-1.5 rounded-full text-sm">
                    <Percent className="w-3 h-3 mr-1" /> On Discount
                    <button onClick={() => setShowDiscount(false)} className="ml-2 hover:text-amber-800"><X className="w-3 h-3" /></button>
                  </div>
                )}
                {searchQuery && (
                  <div className="inline-flex items-center bg-amber-50 text-amber-700 px-3 py-1.5 rounded-full text-sm">
                    Search: "{searchQuery}"
                    <button onClick={() => setSearchQuery("")} className="ml-2 hover:text-amber-800"><X className="w-3 h-3" /></button>
                  </div>
                )}
                <button onClick={resetFilters} className="inline-flex items-center text-sm text-gray-600 hover:text-amber-600">Clear all</button>
              </div>
            )}
            ...
            {filteredProducts.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-2xl border border-gray-200">
                ...
                <p className="text-gray-600 mb-6 max-w-md mx-auto">
                  {searchQuery ? `No products found for "${searchQuery}". Try different keywords or clear filters.` : "No products match your current filters. Try adjusting your criteria."}
                </p>
                <button onClick={resetFilters} className="px-6 py-3 bg-gradient-to-r from-slate-950 to-amber-500 text-white rounded-lg hover:from-slate-800 hover:to-amber-600 transition-colors font-medium">Clear All Filters</button>
              </div>
            ) : (
              <>
                <div className={`${viewMode === "grid" ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" : "space-y-6"}`}>
                  {paginatedProducts.map((product) => (
                    <ProductCard key={product._id || product.id} product={product} viewMode={viewMode} showQuickView={true} />
                  ))}
                </div>
                  ...
                  <div className="mt-12 pt-8 border-t border-gray-200">
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">Previous</button>
                        {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                          ...
                          return (
                            <button key={pageNum} onClick={() => setCurrentPage(pageNum)} className={`w-10 h-10 flex items-center justify-center rounded-lg transition-colors ${currentPage === pageNum ? "bg-gradient-to-r from-slate-950 to-amber-500 text-white" : "border border-gray-300 hover:bg-gray-50"}`}>
                              {pageNum}
                            </button>
                          );
                        })}
                        <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
                      </div>
                    </div>
                  </div>
                )}
            ...
            <div className="relative w-screen max-w-md">
              <div className="flex h-full flex-col bg-white shadow-xl">
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                  ...
                  <button onClick={() => setShowMobileFilters(false)} className="p-2 text-gray-400 hover:text-gray-500"><X className="w-6 h-6" /></button>
                </div>
...
//     description: "Traditional Regular panjabi designs with intricate embroidery" 
//   },
//   { 
//     slug: "premium-panjabi", 
//     name: "Premium panjabi", 
//     description: "Premium panjabi suits perfect for daily wear" 
//   },
//   { 
//     slug: "luxury-panjabi", 
//     name: "Luxury Panjabi", 
//     description: "Luxury Panjabi collection offering elegance and comfort" 
//   },
// ];
