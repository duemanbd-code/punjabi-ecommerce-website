...
const AllCollectionsSection = () => {
  ...
  // Maximum index we can scroll to (showing last card)
  ...
  // Navigation functions - scroll ONE card at a time
  ...
  // Auto-scroll functionality - scroll one card every 5 seconds
  useEffect(() => {
  ...
  }, [filteredProducts, currentIndex, cardsPerView]);
      ...
      <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
        ...
        {safeFilteredProducts.length === 0 ? (
          ...
          >
            ...
            {!isMobile && totalCards > cardsPerView && (
                ...
                {isMobile ? (
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1.5">
                      {Array.from({ length: Math.min(totalCards - cardsPerView + 1, 5) }).map(
                        (_, index) => (
                          <button
                            key={index}
                      ...
                      {totalCards - cardsPerView + 1 > 5 && (
                        <span className="text-xs text-gray-500 ml-1">
                          {currentIndex + 1}/{totalCards - cardsPerView + 1}
                        </span>
                      )}
                    </div>
...
// // client/src/components/AllCollectionsSection.tsx - COMPLETE UPDATED FILE WITH PAGINATION
...
//   // Responsive cards per view
//   const getCardsPerView = useCallback(() => {
//     if (typeof window === "undefined") return { total: 4 };
//     const width = window.innerWidth;
//     if (width < 640) return { total: 1 };
//     if (width < 768) return { total: 2 };
//     if (width < 1024) return { total: 2 };
//     if (width < 1280) return { total: 3 };
//     return { total: 4 };
//   }, []);
...
//     return Math.max(1, Math.ceil(productsArray.length / getCardsPerView().total));
//   }, [filteredProducts, getCardsPerView]);
...
//     autoScrollRestoreTimeout.current = setTimeout(() => setIsAutoScrolling(true), 3000);
//   }, [prevSlide]);
...
//   useEffect(() => {
//     const productsArray = Array.isArray(filteredProducts) ? filteredProducts : [];
//     if (!isAutoScrolling || productsArray.length === 0 || totalSlides <= 1) {
//       if (autoScrollInterval.current) {
//         clearInterval(autoScrollInterval.current);
//         autoScrollInterval.current = null;
//       }
...
//       }
//     };
//   }, [isAutoScrolling, filteredProducts, totalSlides, nextSlide]);
...
//   }, [currentSlide, filteredProducts, getCardsPerView]);
...
//           <div className="text-center mb-8 sm:mb-12">
//             <div className="inline-flex items-center px-3 py-1.5 sm:px-4 sm:py-2 bg-gradient-to-r from-slate-950 to-amber-500 rounded-full mb-3 sm:mb-4">
//               <span className="text-white text-xs sm:text-sm md:text-md font-semibold">All Collections</span>
//             </div>
//             <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 mb-4 sm:mb-6">
//               Our <span className="text-amber-600">Collections</span>
//             </h1>
...
//           <div className="text-center mb-8 sm:mb-12">
//             <div className="bg-red-50 border border-red-200 rounded-lg sm:rounded-xl p-6 max-w-md mx-auto">
//               <p className="text-red-600 font-medium text-sm sm:text-base md:text-lg mb-4">Error: {error}</p>
...
//           {safeFilteredProducts.length > 0 && !isMobile && (
//             <div className="hidden sm:block absolute top-0 right-0">
//               <Link href="/all-collections" className="inline-flex items-center px-3 py-1.5 sm:px-4 sm:py-2 md:px-5 md:py-2.5 bg-gradient-to-r from-slate-900 to-slate-700 hover:from-slate-950 hover:to-slate-800 text-white hover:text-amber-500 text-xs sm:text-sm md:text-base font-semibold rounded-full shadow-lg hover:shadow-xl">
//                 View All
//                 <ArrowRight className="w-3 h-3 sm:w-4 sm:h-4 ml-2 transition-transform duration-300 group-hover:translate-x-1" />
//               </Link>
//             </div>
...
//           {isMobile && (
//             <div className="mb-4">
//               <button onClick={() => setShowCategoryFilter(!showCategoryFilter)} className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-slate-950 to-amber-500 text-white rounded-lg font-medium shadow-lg">
//                 <Filter className="w-4 h-4" />
...
//                   All Products
//                 </button>
//                 {safeCategories.filter((cat) => cat !== "all").map((category) => (
//                   <button
//                     key={category}
//                     onClick={() => setSelectedCategory(category)}
//                     className={`px-3 py-1.5 sm:px-4 sm:py-2 md:px-6 md:py-3 rounded-full font-medium transition-all duration-300 text-xs sm:text-sm md:text-base ${
...
//                     className={`p-3 rounded-lg text-sm font-medium transition-all ${
//                       selectedCategory === "all" ? "bg-gradient-to-r from-slate-950 to-amber-500 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
//                     }`}
//                   >
//                     All Products
//                   </button>
...
//                     <button
//                       key={category}
//                       onClick={() => { setSelectedCategory(category); setShowCategoryFilter(false); }}
//                       className={`p-3 rounded-lg text-sm font-medium transition-all ${
//                         selectedCategory === category ? "bg-gradient-to-r from-slate-950 to-amber-500 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
//                       }`}
//                     >
//                       {category.charAt(0).toUpperCase() + category.slice(1)}
//                     </button>
...
//           <div className="text-center">
//             <p className="text-gray-600 text-sm sm:text-base">
//               Showing {safeFilteredProducts.length} {selectedCategory === "all" ? "products" : selectedCategory + " products"}
//             </p>
//           </div>
//         </div>
...
//           <div className="text-center py-8 sm:py-12 md:py-16 bg-white rounded-xl sm:rounded-2xl shadow-sm">
//             <p className="text-gray-600 text-sm sm:text-base mb-4 sm:mb-6">
//               {selectedCategory === "all" ? "No products available at the moment." : `No products found in the ${selectedCategory} category.`}
//             </p>
//             {selectedCategory !== "all" && (
//               <button onClick={() => setSelectedCategory("all")} className="px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-slate-950 to-amber-500 text-white rounded-lg">
//                 View All Products
//               </button>
//             )}
//           </div>
...
//                 {isMobile ? (
//                   <div className="flex items-center gap-3">
//                     <button onClick={handlePrevSlide} disabled={currentSlide === 0} className="p-2 rounded-full hover:bg-gray-100 transition-colors disabled:opacity-50">
//                       <ChevronLeft className="w-5 h-5 text-gray-600" />
//                     </button>
//                     <div className="flex items-center gap-1.5">
...
//                           onClick={() => goToSlide(index)}
//                           className={`transition-all duration-300 ${
//                             index === currentSlide ? "w-6 h-2 bg-amber-500 rounded-full" : "w-1.5 h-1.5 bg-gray-300 rounded-full hover:bg-gray-400"
//                           }`}
//                         />
//                       ))}
//                     </div>
//                     <button onClick={handleNextSlide} disabled={currentSlide === totalSlides - 1} className="p-2 rounded-full hover:bg-gray-100 transition-colors disabled:opacity-50">
//                       <ChevronRight className="w-5 h-5 text-gray-600" />
//                     </button>
//                   </div>
//                 ) : (
...
//                           onClick={() => goToSlide(index)}
//                           className={`transition-all duration-300 ${
//                             index === currentSlide ? "w-8 h-2 bg-amber-500 rounded-full" : "w-2 h-2 bg-gray-300 rounded-full hover:bg-gray-400"
//                           }`}
//                         />
//                       ))}
//                     </div>
...
//             {isMobile && safeFilteredProducts.length > 0 && (
//               <div className="flex justify-center mt-6">
//                 <Link href="/all-collections" className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-slate-900 to-slate-700 hover:from-slate-950 hover:to-slate-800 text-white hover:text-amber-500 font-semibold rounded-full shadow-lg hover:shadow-xl text-base">
//                   View All Products
//                   <ArrowRight className="w-5 h-5 ml-2 transition-transform duration-300 group-hover:translate-x-1" />
//                 </Link>
//               </div>
