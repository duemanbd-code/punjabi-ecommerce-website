...
export default function AllCollectionsSection() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Fetch ALL products (not just best selling)
  ...
  useEffect(() => {
    if (activeFilter === "all") {
      setFilteredProducts(products.slice(0, 12)); // Show 12 products for carousel
    } else {
      // Filter by category
  ...
  if (loading) {
    return (
      <section id="all-collections" className="py-8 sm:py-12 md:py-16 bg-white">
  ...
  return (
    <section id="all-collections" className="py-8 sm:py-12 md:py-16 bg-white relative">
      <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
        {/* Section Header */}
          ...
          {isMobile && (
            <div className="mb-4">
              ...
              >
                ...
                Filter: {activeFilter === "all" ? "All Products" : categories.find(c => c.key === activeFilter)?.label || activeFilter}
