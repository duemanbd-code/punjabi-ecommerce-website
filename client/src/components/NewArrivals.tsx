// client/src/components/NewArrivals.tsx - COMPLETE UPDATED FILE WITH PAGINATION
...
import { fetchNewArrivalsProducts } from "@/lib/utils/product.api";
...
const NewArrivals = () => {
  ...
  // Carousel state - CHANGED TO currentIndex for one-by-one scrolling
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const result = await fetchNewArrivalsProducts(1, 50);
        ...
        const allCategories: Category[] = [{ key: "all", label: "All New Arrivals" }];
        ...
        newArrivals.forEach((product) => {
          if (product.category) {
            ...
            if (!uniqueCategories.has(categoryKey)) {
              ...
              allCategories.push({ key: categoryKey, label: categoryLabel });
            }
          }
        });
  ...
  useEffect(() => {
    ...
    if (activeFilter === "all") {
      ...
      const categoryProducts = productsArray
        .filter((product) => product.category?.toLowerCase() === activeFilter.toLowerCase());
  ...
  // Navigation functions - ONE CARD AT A TIME
  ...
  const handlePrevSlide = useCallback(() => {
    ...
    if (autoScrollRestoreTimeout.current) clearTimeout(autoScrollRestoreTimeout.current);
    autoScrollRestoreTimeout.current = setTimeout(() => setIsAutoScrolling(true), 8000);
  }, [prevSlide]);
  ...
  useEffect(() => {
    if (!isAutoScrolling || totalCards <= cardsPerView) {
    ...
    autoScrollInterval.current = setInterval(nextSlide, 5000); // Changed to 5 seconds
    return () => {
      if (autoScrollInterval.current) {
        clearInterval(autoScrollInterval.current);
        autoScrollInterval.current = null;
  ...
  }, [isAutoScrolling, totalCards, cardsPerView, nextSlide]);
  ...
  const getCurrentSlideProducts = useCallback(() => {
    return filteredProducts.slice(currentIndex, currentIndex + cardsPerView);
  ...
  if (loading) {
    return (
      <section id="new-arrivals" className="py-8 sm:py-12 md:py-16 bg-gradient-to-b from-white to-gray-50">
        <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
          <div className="text-center mb-8 sm:mb-10 md:mb-12">
            <div className="inline-flex items-center px-3 py-1.5 sm:px-4 sm:py-2 bg-gradient-to-r from-slate-950 to-amber-500 rounded-full mb-3 sm:mb-4">
              <span className="text-white text-xs sm:text-sm md:text-md font-semibold">New Arrivals</span>
            </div>
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 mb-3 sm:mb-4">
              Fresh <span className="bg-gradient-to-r from-slate-950 to-amber-500 bg-clip-text text-transparent">Collection</span>
            </h2>
        ...
        <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
          <div className="text-center mb-8 sm:mb-10 md:mb-12">
            <div className="bg-red-50 border border-red-200 rounded-lg sm:rounded-xl p-4 sm:p-6 max-w-md mx-auto">
              ...
              <button onClick={() => window.location.reload()} className="px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-slate-950 to-amber-500 text-white rounded-lg">
                Try Again
              </button>
            </div>
          </div>
  ...
  const visibleProducts = getCurrentSlideProducts();
      ...
      <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
        <div className="relative mb-8 sm:mb-10 md:mb-12">
          ...
          {safeFilteredProducts.length > 0 && !isMobile && (
            <div className="absolute top-0 right-0">
              <Link href="/new-arrivals" className="inline-flex items-center px-3 py-1.5 sm:px-4 sm:py-2 md:px-5 md:py-2.5 bg-gradient-to-r from-slate-900 to-slate-700 hover:from-slate-950 hover:to-slate-800 text-white hover:text-amber-500 text-xs sm:text-sm md:text-base font-semibold rounded-full shadow-lg hover:shadow-xl">
                View All
        ...
        {safeCategories.length > 1 && (
          <div className="mb-6 sm:mb-8 md:mb-12" ref={categoryFilterRef}>
            {isMobile ? (
              <div className="mb-4">
                <button onClick={() => setShowCategoryFilter(!showCategoryFilter)} className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-slate-950 to-amber-500 text-white rounded-lg font-medium shadow-lg text-sm sm:text-base">
                  <Filter className="w-4 h-4" />
                  Filter: {activeFilter === "all" ? "All New Arrivals" : safeCategories.find(c => c.key === activeFilter)?.label || activeFilter}
              ...
              <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
                {safeCategories.map((category) => (
                  <button
                    ...
                    className={`px-3 py-1.5 sm:px-4 sm:py-2 md:px-5 md:py-2.5 rounded-full font-medium transition-all duration-300 text-xs sm:text-sm md:text-base ${
                      activeFilter === category.key
                        ? "bg-gradient-to-r from-slate-950 to-amber-500 text-white shadow-lg scale-105"
                ...
                <div className="p-4">
                  <div className="grid grid-cols-2 gap-2">
                    {safeCategories.map((category) => (
                      <button
                        key={category.key}
                        onClick={() => { setActiveFilter(category.key); setShowCategoryFilter(false); }}
        ...
        >
          ...
          {!isMobile && totalSlides > 1 && (
            <div className="absolute -top-12 right-0 z-10">
              <button onClick={toggleAutoplay} className="flex items-center gap-2 px-3 py-1.5 bg-white/90 backdrop-blur-sm border border-gray-200 rounded-lg text-xs font-medium text-gray-600 hover:bg-white hover:shadow-lg transition-all shadow-sm">
                {isAutoScrolling ? <><Pause className="w-3 h-3" /> Pause</> : <><Play className="w-3 h-3" /> Play</>}
              </button>
            </div>
          )}
          ...
          {totalSlides > 1 && (
            <>
              ...
              >
                <ChevronLeft className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5 sm:w-6 sm:h-6'} text-gray-700`} />
              </button>
              ...
              >
                <ChevronRight className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5 sm:w-6 sm:h-6'} text-gray-700`} />
              </button>
            </>
          )}
          ...
          <div className="overflow-hidden">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 md:gap-8 transition-all duration-500 ease-in-out">
              {visibleProducts.map((product) => (
                <div key={product._id || product.id} className="animate-fadeIn">
                  <ProductCard product={product} viewMode="grid" showQuickView={!isMobile} />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mt-6 sm:mt-8">
            {isMobile ? (
              <div className="flex items-center gap-3">
                <button onClick={handlePrevSlide} className="p-2 rounded-full hover:bg-gray-100 transition-colors">
                ...
                <button onClick={handleNextSlide} className="p-2 rounded-full hover:bg-gray-100 transition-colors">
        ...
        {isMobile && safeFilteredProducts.length > 0 && (
          <div className="flex justify-center mt-6">
            <Link href="/new-arrivals" className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-slate-900 to-slate-700 hover:from-slate-950 hover:to-slate-800 text-white hover:text-amber-500 font-semibold rounded-full shadow-lg hover:shadow-xl text-base">
              View All Arrivals
...
//         // Extract unique categories from new arrivals
//         const uniqueCategories = new Set<string>();
//         const allCategories: Category[] = [
//           { key: "all", label: "All New Arrivals" },
//         ];
...
//               </span>
//             </h2>
//             <p className="text-sm sm:text-base md:text-lg lg:text-xl text-gray-600 max-w-4xl mx-auto mb-4 sm:mb-6 md:mb-8 px-4">
//               Discover our latest additions. Fresh styles, exclusive designs,
//               and premium quality just arrived.
//             </p>
//           </div>
...
//               >
//                 <Filter className="w-4 h-4" />
//                 Filter: {activeFilter === "all" ? "All New Arrivals" : categories.find(c => c.key === activeFilter)?.label || activeFilter}
//                 <ChevronRight className={`w-4 h-4 transition-transform ${showCategoryFilter ? "rotate-90" : ""}`} />
//               </button>
//             </div>
//           )}
...
//           <div className="text-center py-8 sm:py-12 bg-white rounded-lg sm:rounded-2xl shadow-sm">
...
//                 : `No new products found in the ${activeFilter} category.`}
//             </p>
//             {activeFilter !== "all" && (
//               <button
...
//                     className="flex items-center gap-2 px-3 py-1.5 bg-white/90 backdrop-blur-sm border border-gray-200 rounded-lg 
//                       text-xs font-medium text-gray-600 hover:bg-white hover:shadow-lg transition-all shadow-sm"
//                     title={
